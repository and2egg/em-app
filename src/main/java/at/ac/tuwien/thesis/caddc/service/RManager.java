...
public class RManager {
	...
	public String[] getForecasts(Long locationId, String startDateString, String endDateString) throws REXPMismatchException, REngineException {
		RConnection c = new RConnection(server, port);
	    
		System.out.println("before forecasts");
		String[] modelNames = getModelList(c, "forecast", locationId, startDateString, endDateString);
	    ...
	    System.out.println("after forecasts");
	    c.close();
	    return meanValues;
	}
	...
	public String generateForecasts(Long locationId, String startDateString, String endDateString) throws REXPMismatchException, REngineException {
		RConnection c = new RConnection(server, port);
	    ...
	    String[] modelNames = getModelList(c, "models", locationId, startDateString, endDateString);
	    for(String modelName : modelNames) {
			c.eval("load(\"models/"+modelName+"\")");
			String name = modelName.substring(0, modelName.lastIndexOf("."));
			String fcName = "fc_"+name;
	...
	private String[] getModelList(RConnection c, String modelPath, Long locationId, String startDateString, String endDateString) throws REXPMismatchException, REngineException {
	    String path = getClass().getClassLoader().getResource("data").getPath(); // folder "rscripts" in resource directory
	    path = path.substring(1); // remove leading slash
	    ...
	    String pattern = ".*_"+locId+"_.*"; // get modelnames containing the given location id
	...
	public String generateModel(String modelName, String csvData) throws RserveException, REXPMismatchException {
	    ...
	    c.eval("plot(prices_training, type='l')");
	
	    c.eval(modelName+" <- generate_model(prices_training, output=TRUE)");
	...
	public String rTestGenerateModel(String csvData) throws RserveException, REXPMismatchException {
		RConnection c = new RConnection(server, port);
	    ...
	    c.eval("model_prices_training <- generate_model(prices_training, same.lag=TRUE, output=TRUE)");
	    c.eval("save(model_prices_training, file=\"models/model.RData\")");
	    c.close();
